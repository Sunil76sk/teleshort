/** TeleShort v2.1 — Monetag Ad Session Start */
...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const {data:active}=await supabase.from('ad_sessions').select('*').eq('link_id',link.id).eq('visitor_telegram_id',visitorId).in('status',['AD_1_STARTED','AD_2_STARTED','REWARD_ELIGIBLE']).gt('expires_at',new Date().toISOString()).order('created_at',{ascending:false}).limit(1);
    if(active&&active.length){
      const a=active[0];
      const token=createAdChallengeToken({session_id:a.id,short_code:link.short_code||link.short_id,step:a.step,visitor_id:visitorId,ip_hash:ipHash,is_owner:isOwner,is_eligible:a.metadata?.is_eligible??fraudEval.isEligible,min_duration_ms:4500,created_at:Date.now(),expires_at:new Date(a.expires_at).getTime()});
      return sendSuccess(res,{session_id:a.id,short_code:link.short_code||link.short_id,step:a.step,total_steps:2,network:'MONETAG',status:a.status,challenge_token:token,timer_seconds:5,is_owner:isOwner,is_eligible:a.metadata?.is_eligible??fraudEval.isEligible,resumed:true});
    }

    const {data:adSession,error:sessionErr}=await supabase.from('ad_sessions').insert([{link_id:link.id,visitor_telegram_id:visitorId,step:1,network:'MONETAG',status:'AD_1_STARTED',challenge_hash:ipHash,started_at:new Date().toISOString(),expires_at:new Date(expiresAt).toISOString(),metadata:{is_owner:isOwner,is_eligible:fraudEval.isEligible&&!isOwner,ineligible_reason:isOwner?'SELF_CLICK':fraudEval.reason,fraud_score:fraudEval.score,fraud_status:fraudEval.status,ip_hash:ipHash,ua_hash:hashUserAgent(userAgent)}}]).select('id,step,status').single();
    ...
    const challengeToken=createAdChallengeToken({session_id:adSession.id,short_code:link.short_code||link.short_id,step:1,visitor_id:visitorId,ip_hash:ipHash,is_owner:isOwner,is_eligible:fraudEval.isEligible&&!isOwner,min_duration_ms:4500,created_at:Date.now(),expires_at:expiresAt});
    return sendSuccess(res,{session_id:adSession.id,short_code:link.short_code||link.short_id,step:1,total_steps:2,network:'MONETAG',status:'AD_1_STARTED',challenge_token:challengeToken,timer_seconds:5,is_owner:isOwner,is_eligible:fraudEval.isEligible&&!isOwner,ineligible_reason:isOwner?'SELF_CLICK':fraudEval.reason,resumed:false});
  }catch(error){console.error('[Ad Session Start Error]:',error);return sendError(res,error.message||'Error starting ad session',500,'AD_SESSION_START_ERROR');}
};
