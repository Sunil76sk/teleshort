/** TeleShort v2.1 — Monetag Ad Event & State Progression */
...
module.exports=async function handler(req,res){
  ...
  const visitorId=Number(auth.user.id);const parsedStep=parseInt(step,10);
  if(![1,2].includes(parsedStep)) return sendError(res,'Invalid ad step. Must be 1 or 2.',400,'INVALID_STEP');
  const rateLimit=await checkRateLimit(`event_${visitorId}`,'ad_event_submit',20,60);if(!rateLimit.allowed)return sendError(res,'Too many event submissions. Please slow down.',429,'RATE_LIMITED');
  ...
  try{
    const supabase=getSupabaseClient();const now=Date.now();
    ...
    if(new Date(session.expires_at).getTime()<now){await supabase.from('ad_sessions').update({status:'EXPIRED'}).eq('id',session_id);return sendError(res,'Ad session has expired. Please restart.',410,'SESSION_EXPIRED');}
    ...
    // The provider promise/callback is the completion signal. The timer is only an anti-abuse floor.
    ...
    if(parsedStep===1){
      ...
      await supabase.from('ad_sessions').update({step:2,status:'AD_2_STARTED',metadata:updatedMetadata,updated_at:new Date().toISOString()}).eq('id',session_id);
    ...
    await supabase.from('ad_sessions').update({status:'REWARD_ELIGIBLE',completed_at:new Date(now).toISOString(),metadata:updatedMetadata,updated_at:new Date().toISOString()}).eq('id',session_id);
  ...
  }catch(error){console.error('[Ad Event Error]:',error);return sendError(res,error.message||'Error processing ad event',500,'AD_EVENT_ERROR');}
};
