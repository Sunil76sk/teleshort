...
module.exports = async function handler(req, res) {
  ...
  // IMPORTANT: Telegram user.id is NOT the database users.id.
  // users.id is BIGSERIAL, while Telegram's ID is stored in users.telegram_id.
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      // Resolve Telegram ID -> internal BIGSERIAL user ID.
      const { data: userRecord, error: userErr } = await supabase
        .from('users')
        .select('id, telegram_id, status, is_blocked')
      ...
      if (userRecord.status === 'BANNED' || userRecord.status === 'SUSPENDED' || userRecord.is_blocked === true) {
        return sendError(res, `Account is ${(userRecord.status || 'restricted').toLowerCase()}. Cannot create links.`, 403, 'ACCOUNT_RESTRICTED');
      }
      ...
      if (!insertedLink) return sendError(res, 'Failed to generate unique link code after retries', 500, 'SLUG_COLLISION_FAILED');
      ...
      return sendError(res, error.message || 'Failed to create link', 500, 'LINK_CREATE_ERROR');
    }
  }
  ...
  if (req.method === 'GET') {
    try {
      const supabase = getSupabaseClient();
      const { data: userRecord, error: userErr } = await supabase
        .from('users').select('id').eq('telegram_id', telegramUserId).maybeSingle();
      if (userErr) throw userErr;
      if (!userRecord) return sendError(res, 'User account not found. Please reopen the Telegram Mini App.', 404, 'USER_NOT_FOUND');
      ...
      const { data: links, error, count } = await supabase
        .from('links').select('*', { count: 'exact' }).eq('owner_id', userRecord.id)
      ...
      const formattedLinks = (links || []).map(l => {
        ...
        return {
          id: l.id, short_code: l.short_code, short_url: linkUrl, deep_link: linkUrl,
          original_url: l.original_url, status: l.status, click_count: l.click_count,
          clicks_count: l.click_count, eligible_click_count: l.eligible_click_count,
          total_earnings: l.total_earnings, earnings: l.total_earnings, created_at: l.created_at
        };
      });
      return sendSuccess(res, {
      ...
      return sendError(res, error.message || 'Failed to fetch links', 500, 'LINK_LIST_ERROR');
    }
  }
