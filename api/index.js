...
module.exports = async function handler(req, res) {
  ...
  // WHATWG URL avoids Node's deprecated url.parse() runtime warning.
  const parsedUrl = new URL(req.url || '/', 'http://localhost');
  const pathname = parsedUrl.pathname.replace(/\/+$/, '') || '/';
  req.query = Object.assign({}, Object.fromEntries(parsedUrl.searchParams.entries()), req.query || {});

  if (['POST', 'PUT', 'PATCH'].includes(req.method) && (!req.body || typeof req.body !== 'object')) req.body = await parseBody(req);
  ...
  try {
    ...
    if (pathname === '/api/ad-session/start') return await loadHandler('../server/ad-session/start')(req, res);
    if (pathname === '/api/ad-session/event') return await loadHandler('../server/ad-session/event')(req, res);
    ...
    if (pathname === '/api/admin/auth') return await loadHandler('../server/admin/auth')(req, res);
    ...
    if (adminWithdrawalDecisionMatch) { req.query.id = adminWithdrawalDecisionMatch[1]; return await loadHandler('../server/admin/withdrawals/[id]/decision')(req, res); }
    ...
    if (pathname === '/api/admin/broadcasts') return await loadHandler('../server/admin/broadcasts/index')(req, res);
