...
try { require('../server/route-bundle'); } catch (error) { console.warn('[Route Bundle Warning]:', error.message); }

async function parseBody(req) {
  if (req.body && typeof req.body === 'object') return req.body;
  return new Promise((resolve) => { let bodyStr = ''; req.on('data', c => { bodyStr += c; }); req.on('end', () => { try { resolve(bodyStr ? JSON.parse(bodyStr) : {}); } catch (_) { resolve({}); } }); });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (pathname === '/api/health') return await loadHandler('../server/health')(req, res);
    ...
    const linkMatch = pathname.match(/^\/api\/links\/([a-zA-Z0-9_-]+)$/); if (linkMatch) { req.query.id=linkMatch[1]; return await loadHandler('../server/links/[id]')(req,res); }
    ...
    const withdrawalMatch=pathname.match(/^\/api\/withdrawals\/([a-zA-Z0-9_-]+)$/); if(withdrawalMatch){req.query.id=withdrawalMatch[1];return await loadHandler('../server/withdrawals/[id]')(req,res);}
    if (pathname === '/api/referrals' || pathname === '/api/referrals/stats') return await loadHandler('../server/referrals/index')(req,res);
    if (pathname === '/api/admin/auth' || pathname === '/api/admin/auth/login' || pathname === '/api/admin/auth/logout') { if(pathname.endsWith('/logout')) req.body={...(req.body||{}),action:'logout'}; return await loadHandler('../server/admin/auth')(req,res); }
    if (pathname === '/api/admin/dashboard') return await loadHandler('../server/admin/dashboard')(req,res);
    if (pathname === '/api/admin/settings' || pathname === '/api/admin/ad-config') return await loadHandler('../server/admin/settings')(req,res);
    if (pathname === '/api/admin/users') return await loadHandler('../server/admin/users/index')(req,res);
    const adminUserMatch=pathname.match(/^\/api\/admin\/users\/([a-zA-Z0-9_-]+)$/); if(adminUserMatch){req.query.id=adminUserMatch[1];return await loadHandler('../server/admin/users/[id]')(req,res);}
    if (pathname === '/api/admin/links') return await loadHandler('../server/admin/links/index')(req,res);
    const adminLinkMatch=pathname.match(/^\/api\/admin\/links\/([a-zA-Z0-9_-]+)$/); if(adminLinkMatch){req.query.id=adminLinkMatch[1];return await loadHandler('../server/admin/links/[id]')(req,res);}
    if (pathname === '/api/admin/withdrawals') return await loadHandler('../server/admin/withdrawals/index')(req,res);
    const adminWithdrawalDecisionMatch=pathname.match(/^\/api\/admin\/withdrawals\/([a-zA-Z0-9_-]+)\/(approve|reject)$/); if(adminWithdrawalDecisionMatch){req.query.id=adminWithdrawalDecisionMatch[1];req.body={...(req.body||{}),status:adminWithdrawalDecisionMatch[2]==='approve'?'APPROVED':'REJECTED'};return await loadHandler('../server/admin/withdrawals/[id]/decision')(req,res);}
    const adminWithdrawalDecisionMatch2=pathname.match(/^\/api\/admin\/withdrawals\/([a-zA-Z0-9_-]+)\/decision$/); if(adminWithdrawalDecisionMatch2){req.query.id=adminWithdrawalDecisionMatch2[1];return await loadHandler('../server/admin/withdrawals/[id]/decision')(req,res);}
    ...
    const sendMatch=pathname.match(/^\/api\/admin\/broadcasts\/([a-zA-Z0-9_-]+)\/send$/); if(sendMatch){req.query.id=sendMatch[1];return await loadHandler('../server/admin/broadcasts/[id]/send')(req,res);}
    const cancelMatch=pathname.match(/^\/api\/admin\/broadcasts\/([a-zA-Z0-9_-]+)\/cancel$/); if(cancelMatch){req.query.id=cancelMatch[1];return await loadHandler('../server/admin/broadcasts/[id]/cancel')(req,res);}
  ...
  } catch(error){ console.error('[Router Exception]:',error); return sendError(res,process.env.NODE_ENV==='production'?'Internal Server Error':(error.message||'Internal Server Error'),500,'INTERNAL_SERVER_ERROR'); }
};
